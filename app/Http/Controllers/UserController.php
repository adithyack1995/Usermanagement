...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        $frndlist1 =Friend_list:: where('user_id', $id)->where('status', '1') ->pluck('friend_id')->toArray();
        $frndlist2 =Friend_list:: where('friend_id', $id)->where('status', '1') ->pluck('user_id')->toArray();
    ...
    public function getfriendrequest(){
        ...
        $id = auth()->user()->id;
        $frndlist =Friend_list:: with('User')->where('friend_id', $id)->
        where('status', '0')->select('*')->get()->toArray();
    ...
    public function myFriends(){
        ...
        $frndlist1 =Friend_list:: where('user_id', $id)->where('status', '1') ->pluck('friend_id')->toArray();
        $frndlist2 =Friend_list:: where('friend_id', $id)->where('status', '1') ->pluck('user_id')->toArray();
        $frndlist = array_merge($frndlist1,$frndlist2);

        foreach($frndlist as $user){

            $friends[] =User:: where('id', $user)->select('id','name')->get()->toArray();
